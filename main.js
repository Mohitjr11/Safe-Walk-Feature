...
function selectTransport(type) {
    ...
    const selectedBtn = Array.from(document.querySelectorAll('.transport-btn')).find(btn => {
        return btn.innerText.includes(getTransportIconByType(type));
    });

    if (selectedBtn) {
        selectedBtn.classList.add('bg-blue-200', 'border-blue-600');
    }
...
function getTransportIconByType(type) {
    const icons = {
        'walking': '🚶',
        'car': '🚗',
        'autorickshaw': '🛺',
...
function updateGuardiansList() {
    ...
    list.innerHTML = guardians.map(phone => `
        <div class="guardian-item bg-green-100 border border-green-400 text-green-800 p-2 rounded flex justify-between items-center">
            <span>${phone}</span>
            <button onclick="removeGuardian('${phone}')" class="text-red-500 hover:underline" >Remove</button>
        </div>
    `).join('');
}
...
function triggerSOS(reason = 'Manual SOS triggered') {
    ...
    const message = `
        SOS ALERT!
        Reason: ${reason}
        Current Location: <a href="${locationLink}" target="_blank" class="text-blue-600 hover:underline">View on Google Maps</a>
...
function notifyGuardians(message) {
    ...
    alertElement.className = 'alert bg-orange-100 border border-orange-400 text-orange-700 px-4 py-2 rounded';
